	...
	const items = useMemo(() => {
		if(!asyncRequesting) {
			return [
				{
					key: 'wallet-balance',
					render: () => <Balance btc={data.balance.btc} usd={data.balance.usd}/>
				},

				{
					...
					render: () => (
						<ChartContainer>
							<Tooltip>
								<TooltipAmount>${new Intl.NumberFormat('en-US', { 
									currency: 'USD',
									compactDisplay: 'short',
									notation: 'compact'
								}).format(data.weekEarning)}</TooltipAmount>
							</Tooltip>
...

				{
					key: 'earning-amount',
					render: () => (
						<EarningAmount>${new Intl.NumberFormat('en-US', { 
							currency: 'USD',
							compactDisplay: 'short',
							notation: 'compact'
						}).format(data.weekEarning)}</EarningAmount>
					)
				},

				{
					key: 'update-payment',
					render: () => (
						<UpdatePayment numberOfLines={2}>{t('wallet.updateSettings')}</UpdatePayment>
					)
				},
